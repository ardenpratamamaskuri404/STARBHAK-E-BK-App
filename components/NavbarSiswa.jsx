...
import { 
  Menu, X, Home, CalendarCheck, History, UserCircle, 
...
export default function NavbarSiswa() {
  ...
  const menuItems = [
    { name: "Beranda", href: "/siswa/home", icon: Home },
    { name: "Ajukan Konseling", href: "/siswa/ajukan", icon: CalendarCheck },
...
const avatarUrl =
  session?.user?.avatar_url && session.user.avatar_url !== ""
    ? session.user.avatar_url
    : "/images/avatar-default.png";

    console.log("SESSION DARI NEXTAUTH:", session);
            ...
            return (
              <Link
                ...
                className={`flex items-center gap-2 transition group 
                  ${Active ? "text-[#0046FF]" : "text-gray-700 hover:text-[#0046FF]"}
                `}
              >
                <item.icon
                  size={19}
                  className={`transition group-hover:scale-110 
                    ${Active ? "text-[#0046FF]" : "text-gray-700 group-hover:text-[#0046FF]"}
                  `}
                />
                {item.name}
              </Link>
...
<div className="relative" ref={dropdownRef}>
  ...
  >
    <div className="w-10 h-10 rounded-full overflow-hidden border-2 border-[#0046FF] transition">
      <img src={avatarUrl} alt="Siswa" className="w-full h-full object-cover" />
    </div>
  ...
  {openProfile && (
    <div className="absolute right-0 mt-3 w-64 bg-white rounded-2xl shadow-2xl border overflow-hidden">
      <div className="p-5 bg-gradient-to-br from-[#0046FF] to-blue-700 text-white">
        <p className="font-bold text-lg">{session?.user?.name}</p>
        <p className="text-sm opacity-90">Siswa STARBHAK</p>
      </div>
      ...
      <Link
        href="/settings"
        className="flex items-center gap-3 px-5 py-4 hover:bg-gray-50 transition border-t"
      >
      ...
      >
        ...
        {/* Mobile Toggle */}
        <button className="md:hidden text-[#0046FF]" onClick={() => setOpenMobile(!openMobile)}>
          ...
          {menuItems.map((item) => {
            ...
            return (
              <Link
                ...
                className={`flex items-center gap-4 py-3 font-medium transition 
                  ${Active ? "text-[#0046FF]" : "text-gray-700 hover:text-[#0046FF]"}
                `}
              >
                <item.icon size={22} />
